  ...
  module V1
    class InvitationsController < ApplicationController
      ...
      def invitation_validation_error(email, role)
        return t('api.v1.errors.invitations.invalid_role') unless ALLOWED_ROLES.include?(role)
        return t('api.v1.errors.invitations.invalid_email') unless email.match?(/\A[^@\s]+@[^@\s]+\.[^@\s]+\z/)
